...
PRS_agg_f=${data_scratch_d}/UKB_PRS_PheWAS/HGIrel5.PRS_PheWAS.tsv.gz
PRS_agg_hits_f=${PRS_agg_f%.tsv.gz}.1e-5.tsv.gz
...
{
    echo "#HGI_case_control HGI_suffix clump_p1 phe estimate SE z_or_t_value P" | tr ' ' '\t'
    for HGI_case_control in ${HGI_case_controls[@]} ; do
    for HGI_suffix in ${HGI_suffices[@]} ; do
    for clump_p1 in ${clump_p1s[@]} ; do

        full_f=${data_scratch_d}/UKB_PRS_PheWAS/$(echo ${UKB_cal_f%.tsv.gz} | sed -e "s/@@HGI_case_control@@/${HGI_case_control}/g" | sed -e "s/@@HGI_suffix@@/${HGI_suffix}/g").clump${clump_p1}.PRS_PheWAS_full.tsv.gz

        zcat ${full_f} | grep 'scale(PRS)' | cut -f1,4- \
        | awk -v cc=${HGI_case_control} -v sx=${HGI_suffix} -v p1=${clump_p1} -v OFS='\t' '{print cc, sx, p1, $0}'
...
rename('HGI_case_control' = '#HGI_case_control', 'GBE_ID' = 'phe') %>%
left_join(phe_info_df, by='GBE_ID') %>%
select(
    HGI_case_control, HGI_suffix, clump_p1,
...
zcat ${PRS_agg_f} | awk 'NR == 1 || $NF < 1e-5' | bgzip -l9 -@6 > ${PRS_agg_hits_f}
