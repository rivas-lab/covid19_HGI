...
for clump_p1 in ${clump_p1s[@]} ; do
    ...
    full_f=${data_scratch_d}/UKB_PRS_PheWAS/$(basename ${out_d}).PRS_PheWAS_full.tsv.gz

    if [ ! -d $(dirname ${full_f}) ] ; then mkdir -p $(dirname ${full_f}) ; fi

    if [ -d ${out_d} ] && [ ! -f ${full_f} ] ; then
        cat ${out_d}/$(basename ${out_d}).batch1.tsv | egrep '^#'
